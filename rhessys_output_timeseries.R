...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    sliderInput(inputId = "date_range", 
                ...
                min = happy$bd$date[1], max = happy$bd$date[length(happy$bd$date)]),
    
    #    dateRangeInput("date_range", "Please select date range", 
#                   start=happy$bd$date[1],
#                   end=happy$bd$date[length(happy$bd$date)]),
